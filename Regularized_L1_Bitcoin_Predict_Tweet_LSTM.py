...
# 4 layers
model = Sequential()
model.add(LSTM(units=units,activation='tanh', bias_regularizer = regularizers.l1(0.1), input_shape=(step_size,nb_features),return_sequences=False))
...
callbacks=[CSVLogger(output_file_name+'.csv', append=True),ModelCheckpoint('weights/'+output_file_name+'-{epoch:02d}.hdf5', monitor='val_loss', verbose=1,mode='min')])
...
for i in range(original_datas.shape[1]):
    ...
    predicted_inverted.append(factor_scaling.inverse_transform(predicted[:,:]))
...
plt.title('LSTM With L1 Regularization')
plt.xlabel('Timestamp')
plt.ylabel('Bitcoin Prices')
plt.savefig("LSTM_L1_Reg.png")
